...
def test_parse_data_line_basic(basic_line, source_key):
    # Act
    result, video_start, video_events = parse_data_line(
        basic_line, source_key, None, []
    )
    ...
    assert float(result["lat"]) == -41.2345
    assert float(result["lon"]) == 174.9876
    assert float(result["speed"]) == 2.5
    assert float(result["course"]) == 180.0
    assert float(result["depth"]) == 100.5
    assert float(result["heading"]) == 45.0
...
def test_parse_data_line_media_types(
    ...
    # Assert
    assert result["mediatype"] == expected_media_type
    assert result.get("media", "") == expected_media_path
